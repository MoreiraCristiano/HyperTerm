...
internal sealed class PortaPtySession : IPtySession
{
    ...
    private readonly IPtyConnectionAdapter connection;
    ...
    public PortaPtySession(IPtyConnectionAdapter connection, ILogger logger)
    {
        ArgumentNullException.ThrowIfNull(connection);
        ArgumentNullException.ThrowIfNull(logger);
        this.connection = connection;
        this.logger = logger;
        connection.Exited += OnProcessExited;
    ...
    private async Task ReadOutputAsync(CancellationToken cancellationToken)
    {
        try
        {
            await Utf8PtyOutputReader
                .ReadAsync(connection.ReaderStream, RaiseOutput, cancellationToken)
                .ConfigureAwait(false);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
    ...
    private void TryKillDuringDisposal()
    {
        try
        {
            connection.Kill();
